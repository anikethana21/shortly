"""Kafka producer for link-service — fire-and-forget, non-blocking."""
import asyncio
import json
import logging
import os
...
def fire_and_forget(topic: str, message: dict[str, Any]) -> None:
    """Schedule a Kafka send as a background task. Never blocks the caller."""
    asyncio.create_task(_send(topic, message))


async def _send(topic: str, message: dict[str, Any]) -> None:
    if _producer is None:
        logger.debug("Kafka unavailable, dropping message to %s", topic)
        return
    try:
        await _producer.send_and_wait(topic, message)
    except Exception as exc:
        logger.warning("Kafka send to %s failed: %s", topic, exc)
